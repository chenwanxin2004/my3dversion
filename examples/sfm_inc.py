...
def main():
    ...
    # 直接加载图像文件
    for i in range(5):  # 加载00.jpg到04.jpg
        img_path_single = f"data/relief/{i:02d}.jpg"
        img = cv2.imread(img_path_single)
        if img is None:
            print(f"无法加载图像: {img_path_single}")
            continue
    ...
    for i in range(len(img_set)):
        for j in range(i+1, len(img_set)):
            ...
            if SHOW_MATCH:
                # 创建匹配掩码
                matches_mask = np.zeros(len(match), dtype=np.uint8)
                for idx, m in enumerate(match):
                    if idx < len(inlier_mask) and inlier_mask[idx]:
                        matches_mask[idx] = 1
                
                match_image = cv2.drawMatches(img_set[i], img_keypoints[i], img_set[j], img_keypoints[j], match, None, 
